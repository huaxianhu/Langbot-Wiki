...
The LangBot repository ships a ready-to-use Kubernetes manifest at `docker/kubernetes.yaml`, together with a full [`README_K8S.md`](https://github.com/langbot-app/LangBot/blob/master/docker/README_K8S.md). This page summarizes the essentials, focusing on the **Box sandbox runtime** under Kubernetes — the part that differs most from the Docker Compose deployment.
...
For production, expose via Ingress + TLS — see `README_K8S.md`.
...
- Full manifest and operations: [`docker/README_K8S.md`](https://github.com/langbot-app/LangBot/blob/master/docker/README_K8S.md) in the repo
