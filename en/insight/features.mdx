...
LangBot is an **open-source, production-grade, LLM-native instant messaging bot development platform**. It connects large language models to chat platforms, letting you build intelligent agents — that converse, call tools, and plug into your existing workflows — from a single codebase, all configured and monitored through a browser-based web panel.

LangBot delivers an out-of-the-box IM bot development experience with built-in **Agent, RAG knowledge base, MCP, Skills and a code sandbox**, supports the world's mainstream instant messaging platforms, integrates deeply with LLMOps platforms such as Dify, Coze, n8n and Langflow, and exposes rich HTTP APIs and a production-grade plugin system for custom development.
...
- **RAG Knowledge Base** — built-in vector retrieval with Rerank re-scoring, so your bot answers from your private knowledge.
...
- **Deep LLMOps Integration** — native connectors for Dify, Coze, n8n, Langflow, Deerflow, Aliyun Bailian, Ant TBox and other orchestration & workflow platforms.
...
Since 4.x, LangBot uses **LiteLLM as a unified model request backend**, so beyond the built-in providers below you can also connect any OpenAI-format or LiteLLM-supported model.
...
| [Volc Engine Ark](https://console.volcengine.com/ark) | ✅ | LLM gateway (MaaS) / LLMOps platform |
...
### LLMOps / Agent Orchestration Platforms
...
| RAG (Knowledge Base) | ✅ | Uses Chroma as the vector database, supports Rerank re-scoring |
